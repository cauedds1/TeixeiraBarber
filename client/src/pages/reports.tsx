...
function PeriodFilter({ period, setPeriod, customStart, setCustomStart, customEnd, setCustomEnd }: FilterProps) {
  return (
      ...
      {period === "custom" && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          <input
            ...
            className="bg-[#1a1a1a] border border-white/10 text-white/70 text-xs rounded-lg px-3 py-2 outline-none focus:border-[#C9A24D]/50"
          />
...
function CustomPeriodPlaceholder() {
  return (
...
function TabOverview({ q, ...filter }: { q: string } & FilterProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      {q.length === 0 ? <CustomPeriodPlaceholder /> : (
        <>
                      ...
                      return (
                        <div key={idx} className="flex items-center gap-3">
                          ...
                          <div className="flex-1 h-4 rounded-full bg-white/5 overflow-hidden">
                            <div className="h-full rounded-full" style={{ width: `${pct}%`, background: GOLD, opacity: 0.7 }} />
                          </div>
...
function TabBarbers({ q, ...filter }: { q: string } & FilterProps) {
...
function TabProducts({ q, ...filter }: { q: string } & FilterProps) {
...
function TabServices({ q, ...filter }: { q: string } & FilterProps) {
...
export default function Reports() {
  ...
  const filterProps: FilterProps = { period, setPeriod, customStart, setCustomStart, customEnd, setCustomEnd };
  ...
  return (
    <div className="min-h-screen bg-[#0e0e0e] text-white">
      <div className="p-6 space-y-6 max-w-7xl mx-auto">
        ...
        <div>
          <div className="flex gap-1 bg-[#141414] border border-white/5 rounded-xl p-1 w-fit mb-6">
            {TABS.map(tab => {
              const Icon = tab.icon;
          ...
          {activeTab === "overview" && <TabOverview q={query} {...filterProps} />}
          {activeTab === "barbers" && <TabBarbers q={query} {...filterProps} />}
          {activeTab === "products" && <TabProducts q={query} {...filterProps} />}
          {activeTab === "services" && <TabServices q={query} {...filterProps} />}
        </div>
      </div>
    </div>
  );
